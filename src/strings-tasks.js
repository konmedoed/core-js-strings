...
function removeFirstOccurrences(str, value) {
  ...
  const substr = str.substring(index, index + value.length);
...
function removeLastOccurrences(str, value) {
  ...
  const newStr = rStr.replace(rValue, '');
...
function countVowels(str) {
  ...
  const arr = str.split('');
  for (let i = 0; i < str.length; i += 1) {
    const result = arr[i].search(/^[aeiouy]$/i);
    if (result >= 0) sum += 1;
  }
  return sum;
}
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (arr[i].search(/^[ ,/!?]$/i)) newArr.push(arr[i]);
  }
  const srcStr = newArr.join('').toLowerCase();
  const reverseStr = newArr.reverse().join('').toLowerCase();
  return srcStr === reverseStr;
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const brr = arr.map((item) => item.split('').reverse().join(''));
  return brr.join(' ');
}
...
function invertCase(str) {
  ...
  const brr = arr.map((item) => {
    if (item.toLowerCase() === item) return item.toUpperCase();
...
function encodeToRot13(str) {
  ...
  const arr = str.split('').map((item) => {
    ...
    const index = alphabet.findIndex((itm) => (itm === char ? itm : false));
    if (index === -1) return item;
    if (upperCase) return alphabet[(index + 13) % 26].toUpperCase();
